...
#include <random>
#include <unistd.h>
enum state { VALID, MISS_PENDING, INVALID, MODIFIED };
enum read_write_signal { WR, RD };
int LOG(int a) {
  if (a <= 1)
    return 0;
...
class DirectMappedCache {
  ...
  void set_cpu_req(int A, read_write_signal sig) {
    ...
    cpu_req.word_offset =
        ((A >> character_offset_bits) & ((1 << word_offset_bits) - 1));
    int index_bits = LOG(CACHE_SIZE / BLOCK_SIZE);
    cpu_req.index = ((A / BLOCK_SIZE) & ((1 << index_bits) - 1));
    int tag_bits = CACHE_ADDRESS_SIZE - word_offset_bits - index_bits -
                   character_offset_bits;
...
public:
  CPUReq cpu_req;
  DirectMappedCache(int cache_size, int block_size, int word_size, int mem_size,
                    int cache_address_size, int _is_write_back,
                    int _is_write_alloc) {
  ...
  int process_input(int A, read_write_signal sig, int _data = 0) {
    ...
    if (cpu_req.T == read_write_signal::RD) {
      // Read
      if (B.tag == cpu_req.tag &&
          (B.state == state::VALID || B.state == state::MODIFIED)) {
        // Read Hit
        hit++;
      ...
      if (B.tag == cpu_req.tag &&
        ...
        if (is_write_back) {
          ...
          B.data[cpu_req.word_offset] = _data;
          int idx = cpu_req.block_address * (BLOCK_SIZE / WORD_SIZE) +
                    cpu_req.word_offset;
          memory[idx] = _data;
        }
        return _data;
...
class AssociativeCache {
  ...
  void set_cpu_req(int A, read_write_signal sig) {
    ...
    cpu_req.index = 0;
    int tag_bits =
        CACHE_ADDRESS_SIZE - word_offset_bits - character_offset_bits;
...
public:
  CPUReq cpu_req;
  AssociativeCache(int cache_size, int block_size, int word_size, int mem_size,
                   int cache_address_size, int _is_write_back,
                   int _is_write_alloc) {
    CACHE_SIZE = cache_size;
    BLOCK_SIZE = block_size;
  ...
  int process_input(int A, read_write_signal sig, int _data = 0) {
    ...
    int index = -1;
    for (int i = 0; i < CACHE_SIZE / BLOCK_SIZE; i++) {
      if ((blocks[i].state == VALID || blocks[i].state == MODIFIED) &&
          blocks[i].tag == cpu_req.tag) {
        index = i;
        break;
      }
    }
...
class SetAssociativeCache {
  ...
  void set_cpu_req(int A, read_write_signal sig) {
    ...
    cpu_req.tag =
        ((cpu_req.block_address >> LOG(SETS)) & ((1 << tag_bits) - 1));
    cpu_req.T = sig;
    cpu_req.block_offset = A % BLOCK_SIZE;
  }
...
public:
  CPUReq cpu_req;
  SetAssociativeCache(int cache_size, int block_size, int word_size,
                      int mem_size, int cache_address_size, int _associativity,
                      int _is_write_back, int _is_write_alloc) {
    CACHE_SIZE = cache_size;
    BLOCK_SIZE = block_size;
    WORD_SIZE = word_size;
    MEM_SIZE = mem_size;
    ...
    cpu_req = CPUReq(BLOCK_SIZE / WORD_SIZE);
    tag_bits = CACHE_ADDRESS_SIZE - LOG(BLOCK_SIZE / WORD_SIZE) -
               LOG(WORD_SIZE) - LOG(SETS);
  ...
  int process_input(int A, read_write_signal sig, int _data = 0) {
    ...
    int index = -1;
    for (int i = 0; i < ASSOCIATIVITY; i++) {
      if ((blocks[base + i].state == VALID ||
           blocks[base + i].state == MODIFIED) &&
          blocks[base + i].tag == cpu_req.tag) {
        index = i + base;
        break;
      }
    }
...
int main() {
  ...
  int mem_size = 0;
  int cache_size = 0;
  int associativity = 1, block_size = 0, word_size = 0, is_write_back = 0,
      is_write_alloc = 0;
  std::string s;
  ...
  for (int i = 0; i < mem_size / word_size; i++)
    memory[i] = 0;
  auto cache_address_size =
      block_address_bits + block_offset_bits + word_index_size;
  DirectMappedCache d(cache_size, block_size, word_size, mem_size,
                      cache_address_size, is_write_back, is_write_alloc);
  AssociativeCache a(cache_size, block_size, word_size, mem_size,
                     cache_address_size, is_write_back, is_write_alloc);
  SetAssociativeCache sa(cache_size, block_size, word_size, mem_size,
                         cache_address_size, associativity, is_write_back,
                         is_write_alloc);
