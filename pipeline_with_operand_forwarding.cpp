...
public:
  ...
  void fflip(std::string &s) { flip(s); }
...
private:
  GPR_class() { memset(busy, -1, sizeof(busy)); }
};
class DM_class {
public:
...
private:
  DM_class() { memset(busy, -1, sizeof(busy)); }
};

struct Controller {
  int RegRead, AluSrc, AluOp, MemWrite, MemRead, RegWrite, Mem2Reg, Branch,
      Jump, AluSelect;
...
class Instruction {
  ...
  int npc, jpc, rsl1, rsl2, rdl, opcode, func3, func7, imm, rs1, rs2, AluSrc1,
      AluSrc2, AluResult, AluZeroFlag, bpc, tpc, LDResult, AluResultNegative;
  ...
  void Instruction_Fetch() {
    if (pc >= instructions.size() || IFID.stall)
      return;
  ...
  bool Instruction_Decode() {
    if (IDEX.stall || !IFID.valid)
      return false;
    _IDEX_.jpc = IFID.npc - 1 +
                 (ut.get_bits_range(IFID.ir, 6, 0) == JAL_TYPE_OPCODE
                      ? ut.get_bits_range(IFID.ir, 31, 12)
                      : ut.get_bits_range(IFID.ir, 31, 20));
    ...
    if (_IDEX_.cw.RegRead) {
      if (rsl1 != 0 && GPR.busy[rsl1] != -1) {
        if (!((GPR.busy[rsl1] == IDEX.dpc - 1 &&
               ut.get_bits_range(IDEX.ir, 6, 0) != L_TYPE_OPCODE) ||
              (GPR.busy[rsl1] == EXMO.dpc - 1))) {
          IFID.stall = 1;
          return false;
        }
      }
      if (rsl2 != 0 && GPR.busy[rsl2] != -1) {
        if (!((GPR.busy[rsl2] == IDEX.dpc - 1 &&
               ut.get_bits_range(IDEX.ir, 6, 0) != L_TYPE_OPCODE) ||
              (GPR.busy[rsl2] == EXMO.dpc - 1))) {
          IFID.stall = 1;
          return false;
        }
      }
  ...
  int AluSrc1_Forwarder() {
    ...
    } else if (GPR.busy[IDEX.rsl1] != -1 &&
               GPR.busy[IDEX.rsl1] != IDEX.dpc - 1) {
      IDEX.stall = 1;
      IFID.stall = 1;
      return INT_MIN;
    } else {
  ...
  int AluSrc2_Forwarder() {
    ...
    } else if (GPR.busy[IDEX.rsl2] != -1 &&
               GPR.busy[IDEX.rsl2] != IDEX.dpc - 1) {
      IDEX.stall = 1;
      IFID.stall = 1;
      return INT_MIN;
    } else {
