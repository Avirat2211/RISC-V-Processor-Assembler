...
public:
    ...
    void assign(std::vector<std::string>& tok) {
        tokens=tok;
    }
...
public:
    ...
    std::string convert_tokens_to_binary(){
        ...
        ans = imm1+" "+rs2+" "+rs1+" "+func3+" "+imm2+" "+opcode;
        return ans;
    }
private:
...
public:
    ...
    std::string convert_tokens_to_binary(){
        ...
        if (tokens[0]=="SLLI" || tokens[0]=="SRLI" || tokens[0]=="SRAI") {
            ...
            ans=imm+" "+imm2+" "+rs1+" "+func3+" "+rd+" "+opcode;
        }
        else {
            std::string imm=ut.decimal_to_binary(stoi(tokens[3])).substr(20);
            std::string rs1=registers[tokens[2]];
...
public:
    ...
    std::string convert_tokens_to_binary(){
        ...
        std::string offset=ut.decimal_to_binary(label_line-line_number).substr(20);
...
private:    
    B_Type(std::vector<std::string> &tokens, int i){this->tokens=tokens; line_number=i;}  
};
...
public:
    ...
    std::string convert_tokens_to_binary(){
        std::string ans="";
        if (tokens[0]=="JAL") {
            std::string offset=ut.decimal_to_binary(labels[tokens[2]]-line_number).substr(12);
            std::string rd=registers[tokens[1]];
            ...
            std::string offset=ut.decimal_to_binary(label_line-line_number).substr(12);
            std::string rd="00000";
            std::string opcode="1101111";
            ans=offset+" "+rd+" "+opcode;
        }else if(tokens[0]=="JR"){
...
private:    
    J_Type(std::vector<std::string> &tokens, int i){this->tokens=tokens; line_number=i;}  
};
